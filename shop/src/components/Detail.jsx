...
const Detail = (props) => {
    let { paramId } = useParams();// paramId는 문자열로 오기 때문에 숫자로 바꿔줘야 한다. let [tap, setTap] = useState(0);
 ...
 // 훅은 조건문(if) 아래에서 호출하면 안 됨 (React가 Hook 순서 기억을 못함) 
    ...
    return (
        <div className={'container start ' + fade2}>
                ...
                <Button
                    variant="primary"
                    onClick={() => { // dispatch(addItem( {id : 1, imgurl : 'fruit1.jpg', name : 'Grey Yordan', count : 1}))
                    dispatch( addItem({ id: id, imgurl: imgUrl.replace("img/", ""), name: title, count: 1, }) ); }} style={{ marginRight: "10px" }} >
                    주문하기
                    </Button>
                    <Link to="/cart">
...
function TabContent({ tap }) {
    ...
    return (
        ...
        { [<div>내용0</div>, <div>내용1</div>, <div>내용2</div>][tap] } </div>
    )
   }
